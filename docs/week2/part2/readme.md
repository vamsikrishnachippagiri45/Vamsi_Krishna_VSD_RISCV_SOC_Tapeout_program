...
3. [VSDBabySoC Module Overview](#vsdbabysoc-module-overview)
    - [Core IP Components](#core-ip-components)
    - [Peripheral Stubs](#peripheral-stubs)
    - [SoC Wrapper](#soc-wrapper)
    - [RVMYTH Core Source](#rvmyth-core-source)
4. [RVMYTH Core Simulation Methods](#rvmyth-core-simulation-methods)
    - [SandPiper Method](#sandpiper-method)
    - [Stub Method](#stub-method-used-here)
        - [rvmyth_stub Module](#rvmyth_stub-module)
5. [Pre-Synthesis Simulation](#pre-synthesis-simulation)
    - [Compilation (iverilog)](#compilation-iverilog)
    - [Simulation Execution](#simulation-execution)
    - [Waveform Analysis (GTKWave)](#waveform-analysis-gtkwave)
6. [Observations in GTKWave](#observations-in-gtkwave)
    - [RVMYTH Stub Verification](#rvmyth-stub-verification)
        - [Reset and Initialization](#reset-and-initialization)
        - [Clocking and Data Generation](#clocking-and-data-generation)
    - [PLL Verification](#pll-verification)
    - [DAC Verification](#dac-verification)
    - [Top-Level VSDBabySoC Verification](#top-level-vsdbabysoc-verification)
        - [Core Interfaces](#core-interfaces-reset-and-clocking)
        - [Data Path Verification (RVMYTH to DAC)](#data-path-verification-rvmyth-to-dac)
7. [Summary](#summary)
...
OUT (DAC Output): This signal reflects the analog output generated by the DAC's behavioral model in response to the RV_TO_DAC input.
The OUT signal shows a corresponding analog-like stepping or ramping pattern. This validates the entire data path—from the RVMYTH output interface, through the SoC wrapper, and into the DAC peripheral—is correctly connected and fully functional.

The simulation successfully verifies the most critical aspects of the SoC: stable clocking, proper reset sequencing, and reliable digital data transfer between the processor and the output peripheral, all operating synchronously.

## Summary 

In summary, the functional model proves that the major IP blocks of the VSDBabySoC are correctly instantiated and interconnected according to the design specification, paving the way for the next stage of synthesis and physical implementation.
